...
O objetivo desta análise foi verificar o comportamento das exportações de uva do Brasil em janeiro de 2022. Os dados utilizados são mensais e disponibilizados pelo **Comexstat (MDIC)** para o período de *Janeiro de 2016 a Fevereiro de 2022*;

Em relação aos volumes, em `r strftime(atual, format = "%B")` foram de 721 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 234 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 371 t e, em `r strftime(atual, format = "%B")` de 2021, foram 1.165 t, ou seja, houve um aumento de 94,34% em relação a 2020 e uma queda de -38,10% na comparação com o mesmo período de 2021.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram cerca de 1,829 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 638 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 901,5 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 2,954 milhões, ou seja, os valores superam o ano de 2020 mas caem cerca de -38\% em relação ao ano passado.
